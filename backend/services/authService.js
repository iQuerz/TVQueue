...
const register = asyncHandler(async (req, res, next) => {
    ...
    const createdAccount = await _accountContext.create({ email, name, password})

    const token = signToken(createdAccount._id, res)

    res.status(_code.created).json({token, msg: _msg.registeredAccount})
})
...
const login = asyncHandler(async (req, res, next) => {
    ...
    res.status(_code.ok).json({token, msg:_msg.loggedInAccount})
})
